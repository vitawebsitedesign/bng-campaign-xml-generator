...
(function() {
  ...
  function switchToEditorMode() {
    setCampaignXmlHeight();
		getAddHtmlFieldsForXmlPromise().then(afterAddHtmlFieldsForXmlHook);
	}
	
	function afterAddHtmlFieldsForXmlHook() {
  ...
  function getAddHtmlFieldsForXmlPromise() {
  ...
  function getHostFromUrl(url) {
	...
	function showXmlCleanupToast() {
		$('.bng-toast-xml-cleanup').css('display', 'block');
	}

	function hideXmlCleanupToast() {
		const toast = $('.bng-toast-xml-cleanup');
		if (!toast.length) {
			console.warn('Failed to find toast (is the selector correct?)');
		}

		var toastShown = toast.css('animation-name') === 'none';
		if (toastShown) {
			$('.bng-toast-xml-cleanup').css('animation', 'collapseUp 2s ease-out forwards');
		}
	}
